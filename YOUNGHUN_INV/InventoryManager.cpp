...
void InventoryManager::outbound() {
    ...
    cout << "------ 출고 ------" << endl;
    cout << "상품ID   : "; cin >> id;
    cout << "출고개수 : "; cin >> amount;
    for (int i = 0; i < nCurrentIndex; i++) {
        if (pProductArray[i].getId() == id && pProductArray[i].getActive()) {
            pProductArray[i].reduceQuantity(amount);
            cout << "상품출고 완료 (ID : " << id << "), Quantity : "
                 << pProductArray[i].getQuantity() << endl;
            return;
        }
    }
